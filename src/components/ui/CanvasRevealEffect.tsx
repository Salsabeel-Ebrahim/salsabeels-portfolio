...
const DotMatrix: React.FC<DotMatrixProps> = ({
  ...
  const uniforms = useMemo(() => {
    ...
    if (colors.length === 2)
      colorsArray = [...colors.slice(0, 1), ...colors.slice(0, 1), ...colors.slice(1)];
    if (colors.length === 3)
...
const ShaderMaterial = ({
  ...
  const getUniforms = () => {
 const preparedUniforms: Record<string, { value: number | number[] | THREE.Vector2 | THREE.Vector3 | THREE.Vector3[] }> = {};
